#!/usr/bin/env python3
"""
Reapply Overlays with New Layout
Uses the optimized images (without text) and reapplies text overlays with the final layout
"""
...
class OverlayReapplier:
    ...
    def process_all(self):
        ...
        print("🚀 Reapplying overlays with final layout to all 550 covers...")
        ...
        for i, image_data in enumerate(images_data):
            ...
            # Find the input image (without text)
            input_path = os.path.join(self.input_dir, filename)
            if not os.path.exists(input_path):
                print(f"  ⚠️  Could not find input image: {filename}")
                failed_count += 1
                continue
            
            # Apply the layout
            output_path = self.apply_layout(input_path, metadata, filename)
            if output_path:
                processed_count += 1
            else:
                failed_count += 1
...
def main():
    reapplier = OverlayReapplier()
    reapplier.process_all()
